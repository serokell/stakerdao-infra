...
data Target = Staging | Production deriving (Read, Show)

data Deployment = System | Agora deriving (Read, Show, Eq)
...
fetch Opts {..} = run ("nix", ["eval", "--raw", "(builtins.fetchGit { url = ''"<>repo<>"''; ref = ''"<>ref<>"''; })"])
...
deploy = fetch >>- build >>- push<>activate >>- deploy'
  where
    deploy' o _ = when (deployment o == System) $ deploy $ o { deployment = Agora }
